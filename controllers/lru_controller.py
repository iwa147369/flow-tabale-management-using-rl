...
class LRUController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(f"Initialized LRU Controller (max {self.max_flows} flows)")
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        if len(self.flow_table) >= self.max_flows:
            ...
            if evictable:
                lru = min(evictable, key=lambda e: e.get('time', 0))
                # Remove from the real table using identity
                self.flow_table = [f for f in self.flow_table
                                   if f.get('match_key', f.get('match')) !=
                                      lru.get('match_key', lru.get('match'))]
                self.logger.warning(f"Flow table full — removing LRU (legal): {lru.get('match')}")
                self.remove_flow(datapath, lru['match'], lru.get('priority', 1))
            else:
    ...
    def save_trace(self, path: str = None):
        ...
        if path is None:
            ...
            path = f"traces/trace_lru_{timestamp}.pkl"
